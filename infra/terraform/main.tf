module "vpc" {
  ...
  elasticache_subnets             = [for k, v in local.azs : cidrsubnet(var.vpc_cidr, 8, k + 30)]
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "POSTGRES__HOST", value = aws_db_instance.postgres.address },
      { name = "POSTGRES__PORT", value = "5432" },
      { name = "POSTGRES__DATABASE", value = var.db_name },
      { name = "POSTGRES__USER", value = var.db_username },
      { name = "REDIS__URL", value = "redis://${aws_elasticache_cluster.redis.cache_nodes[0].address}:6379/0" },
    ...
    secrets = [
      { name = "POSTGRES__PASSWORD", valueFrom = var.db_password_secret_arn },
